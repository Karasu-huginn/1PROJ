...
def menu():
    while True:
        ...
        rect_jeu_normal = afficher_texte("1. Mode Normal (3 alignements)", 50, hauteur_fenetre // 2.5 + 50, NOIR, police_hover if 50 < mouse_x < 50 + police_hover.size("1. Mode Normal (3 alignements)")[0] and hauteur_fenetre // 2.5 + 50 < mouse_y < hauteur_fenetre // 2.5 + 50 + police_hover.size("1. Mode Normal (3 alignements)")[1] else police)
        rect_jeu_blitz = afficher_texte("2. Mode Blitz (1 alignement)", 50, hauteur_fenetre // 2.5 + 100 + 30, NOIR, police_hover if 50 < mouse_x < 50 + police_hover.size("2. Mode Blitz (1 alignement)")[0] and hauteur_fenetre // 2.5 + 100 + 30 < mouse_y < hauteur_fenetre // 2.5 + 100 + 30 + police_hover.size("2. Mode Blitz (1 alignement)")[1] else police)
        ...
        rect_retour = afficher_texte(texte_retour, x_retour, y_retour, NOIR, police_hover if x_retour < mouse_x < x_retour + largeur_texte_retour and y_retour < mouse_y < y_retour + hauteur_texte_retour else police)
        
        # Récupérer les boutons cliqués de la souris
        mouse_click = pygame.mouse.get_pressed()
        
        # Vérifier si le bouton "Jouer en ligne" est cliqué
        if rect_jeu_normal.collidepoint((mouse_x, mouse_y)) and mouse_click[0] == 1:
            jouer_en_ligne()
        
        # Vérifier si le bouton "Jouer sur ce PC" est cliqué
        if rect_jeu_blitz.collidepoint((mouse_x, mouse_y)) and mouse_click[0] == 1:
            jouer_sur_ce_pc()
